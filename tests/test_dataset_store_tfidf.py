"""Task 2.3 — Verify DatasetStore TF-IDF index and similarity scoring.
...
class TestIndexBuild:
    def test_load_and_index_builds_tfidf_index(self, loaded_store):
        """Index must be built during load_and_index."""
...
class TestReturnType:
    def test_returns_list(self, loaded_store):
        candidates = loaded_store._titles_list[:3]
        result = loaded_store.tfidf_similarity("thriller drama", candidates)
        assert isinstance(result, list)

    def test_returns_scored_title_instances(self, loaded_store):
        candidates = loaded_store._titles_list[:3]
        result = loaded_store.tfidf_similarity("action", candidates)
        assert all(isinstance(item, ScoredTitle) for item in result)

    def test_result_length_equals_candidate_count(self, loaded_store):
        candidates = loaded_store._titles_list[:5]
        result = loaded_store.tfidf_similarity("drama", candidates)
        assert len(result) == 5
...
class TestOrdering:
    def test_results_sorted_descending_by_score(self, loaded_store):
        candidates = loaded_store._titles_list[:]
        result = loaded_store.tfidf_similarity("thriller drama", candidates)
        scores = [r.similarity_score for r in result]
        assert scores == sorted(scores, reverse=True)

    def test_relevant_title_ranks_above_irrelevant(self, loaded_store):
        """s1 description='A gripping thriller.' should outscore s8 ('An anime film.')
        when querying for 'thriller'."""
    ...
    def test_anime_query_ranks_anime_title_higher(self, loaded_store):
        """s8 description='An anime film.' should outscore s1 for query 'anime'."""
        s1 = loaded_store.get_by_id("s1")
        s8 = loaded_store.get_by_id("s8")
...
class TestScoreRange:
    def test_similarity_scores_are_between_zero_and_one(self, loaded_store):
        candidates = loaded_store._titles_list[:]
        result = loaded_store.tfidf_similarity("drama comedy", candidates)
        for item in result:
            assert 0.0 <= item.similarity_score <= 1.0

    def test_exact_title_word_produces_nonzero_score(self, loaded_store):
        """Querying 'Thriller' (the title of s1) should give s1 a non-zero score."""
        s1 = loaded_store.get_by_id("s1")
...
class TestEdgeCases:
    def test_empty_candidate_list_returns_empty(self, loaded_store):
        result = loaded_store.tfidf_similarity("drama", [])
        assert result == []
    ...
    def test_single_candidate_returns_list_of_one(self, loaded_store):
        candidate = [loaded_store.get_by_id("s2")]
        result = loaded_store.tfidf_similarity("adventure series", candidate)
        assert len(result) == 1

    def test_unknown_query_term_returns_zero_scores(self, loaded_store):
        """A word that doesn't appear in any title or description → all zeros."""
        candidates = loaded_store._titles_list[:]
        result = loaded_store.tfidf_similarity("xyzzy-nonexistent-term", candidates)
        assert all(item.similarity_score == 0.0 for item in result)
